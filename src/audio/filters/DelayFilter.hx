...
{ 
    ...
    private function setFeedback(_feedback:Float):Void {
        feedback = _feedback; 
        if (copy) copy.feedback = _feedback;
    }
    private function setUsems(_usems:Bool):Void {
        usems = _usems;
        if (copy) copy.usems = _usems;
    }
    private function setMix(_mix:Float):Void {
        mix = _mix;
        if (copy) copy.mix = _mix;
    }
    private function setLength(_length:Int):Void {
